...
public class TridentDupe extends Module {
    ...
    public final Setting<Item> trident = sgDefault.add(new ItemSetting.Builder()
        .name("trident")
        .description("Surely the trident goes here")
        .defaultValue(Items.TRIDENT)
        .build()
    );
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (currentTick == 1) {
            if (!mc.player.getInventory().getSelectedItem().is(trident.get())) return;
