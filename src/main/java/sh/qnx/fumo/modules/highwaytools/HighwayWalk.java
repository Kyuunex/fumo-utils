...
public class HighwayWalk extends Module {
    ...
    private final Setting<Boolean> walkThroughLava = sgGeneral.add(new BoolSetting.Builder()
        .name("walk-through-lava")
        .description("Walk through lava.")
        .defaultValue(true)
        .build()
    );
    ...
    private boolean canWalkThrough(BlockPos pos) {
        ...
        return state.isAir() || ((walkThroughLava.get() && state.liquid()));
    }
