...
public class ElytraWatch extends Module {
    ...
    private final Setting<Boolean> checkIfLanded = sgGeneral.add(new BoolSetting.Builder()
        .name("check-if-landed")
        .description("Check if landed")
        .defaultValue(false)
        .build()
    );
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (hasLanded() || isDurabilityLow(chestStack)){
            String alertMsg = "Elytra Alarm";
    ...
    private boolean isDurabilityLow(ItemStack chestStack) {
        return chestStack.getMaxDamage() - chestStack.getDamageValue() <= durabilityThreshold.get();
    }

    private boolean hasLanded() {
        if (!checkIfLanded.get()) return false;
