...
public class FumoFly extends Module {
    ...
    private final Setting<Integer> pitchRecovery = sgPCSettings.add(new IntSetting.Builder()
        .name("pitch-recovery")
        .description("How long to way until we start accelerating after aiming downwards")
        .defaultValue(1)
        .range(0, 2000)
        .sliderRange(0, 100)
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (!(mc.player.getItemBySlot(EquipmentSlot.CHEST).getItem() instanceof ElytraItem)) return;
